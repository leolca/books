...
\begin{example}[Divergência de Kullback-Leibler em Textos e Dados Aleatórios]
  ...
  Calculamos $D(p||q)$ para cada amostra e plotamos histogramas das distribuições
  empíricas para visualizar como elas diferem da uniforme.
  Esperamos que, para \texttt{/dev/urandom}, $D(p||q)$ seja próximo de zero,
  refletindo a aleatoriedade dos dados, enquanto para o texto, $D(p||q)$ 
  ...
  Para \texttt{/dev/urandom}, $D(p||q)$ é pequeno e diminui com $n$,
  indicando que a distribuição empírica se aproxima da uniforme.
  Para Ulysses, $D(p||q)$ estabiliza em um valor positivo (aproximadamente 0.5),
