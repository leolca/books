...
A \emph{comunicação}\index{comunicação} envolve alguns componentes fundamentais: uma fonte, que gera a
...
unificada. Para tanto, ele introduziu conceitos fundamentais como \emph{entropia}\index{entropia} e
\emph{capacidade de canal}\index{capacidade de canal}. Neste livro, procuraremos apresentar uma visão geral da
...
\emph{variável aleatória}\index{variável aleatória} (v.a.). Uma variável aleatória é uma variável que pode
assumir diferentes valores dependendo de fatores aleatórios, ou seja, há
...
como, por exemplo, $X$. Uma \emph{variável aleatória discreta}\index{variável aleatória!discreta} é aquela que pode
...
específico que a variável assume será denotado por uma letra minúscula
$x$. Assim, podemos descrever que a variável aleatória $X$ assume o valor
$x$ através da realização do evento $\{X=x\}$. 

Considere uma v.a. $X$, em um alfabeto $\mathcal{X}$ de cardinalidade $N = \vert\mathcal{X}\vert$,
...
\textcite{hartley1928} introduziu a ideia de que a quantidade de informação\index{informação!Hartley} pode
...
consonante com a interpretação da capacidade representacional de
uma unidade de memória. Uma unidade de memória com $n$ bits
...
relação demonstra que, conforme a quantidade de bits de memória aumenta,
apesar do número de sequências possíveis crescer exponencialmente, a
capacidade de representação de informações cresce proporcionalmente ao crescimento
da memória, alinhando-se perfeitamente com a proposta de Hartley de que a informação é
medida em função do logaritmo do número de estados possíveis.

Um conceito fundamental que emerge na discussão sobre a teoria da informação é
a \emph{redundância}\index{redundância}, ou ainda, a previsibilidade. A redundância refere-se à parte da
...
\begin{equation}\label{eq-info-prop}
  I(E_k) = \log_2 (\sfrac{1}{p_k}) = − \log_2 (p_k) . 
\end{equation}
%
...
ocorrência é inesperada. Em contrapartida, eventos com alta probabilidade
...
variável aleatória influencia na informação\index{informação} associada a ela. A
...
expressa por $H(X)$.
\begin{definition}[Entropia\index{entropia}]
...
diferente, expressaremos explicitamente (por exemplo, $\ln = \log_e$ para base $e$ e $\log_{10}$ para base decimal).
...
combinadas para obter um resultado global.

A formulação matemática da entropia de Shannon, dada na \Cref{eq:entropy-def},
emerge naturalmente ao considerar essas propriedades. A função
logarítmica utilizada possui a sua capacidade de transformar multiplicações em
...
\begin{interlude}
  ...
  expressão acima e dividindo por $N \log s$\footnote{
  ...
  \end{equation}
  onde $\epsilon$ é arbitrariamente pequeno, para $N$ arbitrariamente
  grande.
  ...
  podemos concluir que, para $N$ grande suficiente,
  \begin{equation}
...
\begin{example}[Entropia de uma v.a. binária]
Considere uma v.a. binária $X \in \mathcal{X} = \{0,1\}$.
...
\begin{subequations}\label{eq:entropia-binaria}
  \begin{align}
    H(X) &=  -\theta \log \theta - (1-\theta) \log (1-\theta) \\
         &= h(\theta)
  \end{align}
\end{subequations}
onde, para simplificar, denotamos esta grandeza por $h(\theta)$.
Sempre que utilizarmos esta notação, como por exemplo em $h(\sfrac{1}{4})$, $h(\sfrac{1}{3})$ ou $h(\pi)$,
estaremos nos referindo à \emph{entropia binária}\index{entropia!binária} definida na \Cref{eq:entropia-binaria}.
Em alguns casos, pode aparecer a notação $H(\theta)$ para representar 
a entropia binária, sem comprometer o entendimeno, uma vez que ficará claro pelo contexto
se tratar de uma entropia binária.
...
não existindo assim incerteza.
Observe ainda como o máximo é alcançado em $\theta = \sfrac{1}{2}$, quando
ambos eventos são equiprováveis (incerteza máxima). A função observada é
...
\begin{example}[Entropia de uma v.a. uniforme]\index{entropia!distribuição uniforme}
...
$0 \log 0$ no somatório. Como este termo é indefinido, iremos, neste caso,
usar o valor limite $\lim_{x \rightarrow 0} x \log x = 0$.
...
\end{align}
\end{subequations}
Devemos então nos atentar a isto, sobretudo quando realizarmos calculos numéricos
(a função para cálculo da entropia deve tratar estes casos).

\begin{example}[Entropia de uma distribuição de Zipf\index{entropia!Zipf}]
Neste exemplo, vamos buscar aproximar a entropia de textos escritos.
Sabemos que a distribuição das palavras em textos segue uma lei de potência
conhecida como Lei de Zipf
\parencite{zipf1935,zipf1949,FerreriCancho2001,araujo2013}.  A Lei de Zipf é uma
observação empírica que também é observada em diversos outros fenômenos naturais. Ao
...
A entropia\index{entropia!propriedades} possui várias propriedades fundamentais
que nos ajudam a entender seu comportamento e medidas relacionadas,
como também serão uteis em diversas demonstrações.
Nesta seção, exploraremos algumas das principais propriedades da
entropia, incluindo a não negatividade, a concavidade, valor máximo, a
continuidade, e a mudança de base.

Usualmente a unidade escolhida para medir a informação
é bit, e portanto utilizamos a base $2$. Entretanto, a entropia pode ser dada
também em outras bases\index{entropia!bases}: nats (base $e$), trits (base $3$), hartley (base $10$).
Para obtermos a entropia em outras bases, basta utilizar a regra de mudança de
...
Para verificá-la, basta notar que a entropia é a soma de termos da forma
$-p \log p$. Como $0 \leq p \leq 1$, teremos que cada termo da soma é maior ou
igual a zero. Por conseguinte, a entropia será não negativa.
...
formalmente no Lema \ref{lm:desfundamental}.
% script to create the figure
% lnzzm1.R
\begin{marginfigure}%
...
Vamos demonstrar que $\ln z \leq z-1$, para $z > 1$ por contradição.
...
Pelo Teorema do Valor Médio\index{teorema!do valor médio} $\exists c$, $1 < c < b$, tal que
\begin{equation}
...
sendo este a entropia da distribuição uniforme.
\begin{theorem}[Limite superior da entropia\index{entropia!máximo}]
A entropia é limitada superiormente pela entropia da distribuição uniforme:
  \begin{equation}
    H(X) \leq \log N .
  \end{equation}
\end{theorem}
\begin{proof}
Note que, mostrar que $H(X) \leq \log N$
...
H(X) - \log N \: &= - \sum_x p(x) \log p(x)  - \log N \overbrace{\sum_x p(x)}^{=1} \\
    ...
    &= - \sum_x p(x) \log \left(p(x) \, N\right) \\
    &= \log_2 e \sum_x p(x) \ln \frac{1}{p(x) N} \\
    &\leq \log_2 e \sum_x p(x) \left[ \frac{1}{p(x) N} -1 \right] \label{eq:demlimentropy-a}\\
    &= \log_2 e \left[ \underbrace{\sum_{x \in \mathcal{X}} \frac{1}{N}}_{= N \frac{1}{N} = 1} - \underbrace{\sum_x p(x)}_{=1} \right] = 0.
\end{align}
\end{subequations}
Em \ref{eq:demlimentropy-a}, utilizamos a \Cref{eq:fund-ineq}, com $z = \sfrac{1}{p(x) \, N}$.
...
Esta propriedade será demonstrada mais à frente no \Cref{thm:entropiaconcava},
pois antes precisaremos fazer mais algumas definições e demonstrações.
...
\emph{entropia conjunta}\index{entropia!conjunta} e a \emph{entropia condicional}\index{entropia!condicional}
são conceitos que surgem da extensão das definições
...
\begin{definition}[Entropia Conjunta\index{entropia!conjunta}]
...
\end{align}
\end{subequations}
onde, em \ref{eq:jointentropysimpl}, para simplificar\footnote{Tais simplificações podem
...
Dada a definição, vamos abordar um exemplo prático para analisar a
...
\begin{figure}%
    ...
    \caption{Probabilidade conjunta do primeiro e segundo caractere analisando
    os 44 textos de William Shakespeare no Projeto Gutenberg. As entropias
    simples e conjunta são apresentadas, evidenciando que a entropia conjunta é
    maior. Porém, analisando a entropia por caractere, a entropia conjunta
    abarca dependências entre caracteres consecutivos, reduzindo o efeito de
...
\begin{definition}[Entropia Condicional\index{entropia!condicional}]
...
julgamento: uma única evidência pode aumentar a incerteza sobre um
...
Note que, condicionando $H(Y|X)$ a $Z$, usando a formulação da \Cref{eq:entcondevents} podemos
escrever
...
\begin{theorem}[Regra da Cadeia\index{entropia!regra da cadeia}]\label{thm:regracadeia}
 \begin{equation}
  H(X,Y) = H(X) + H(Y|X) = H(Y) + H(X|Y)
 \end{equation}
\end{theorem}
\begin{proof}
 Como $p(x,y) = p(x) p(y|x)$ (Teorema de Bayes\index{teorema!de Bayes}), e como o logaritmo de um produto é a soma 
...
Para calcular a entropia condicional $H(Y|Y)$, devemos calcular a entropia condicional $p(y|x)$:
\begin{equation*}
\begin{array}{c|ccc}
p(y|x) & y_1 & y_2 & y_3 \\
\hline
...
quanto a incerteza\index{incerteza} sobre uma é reduzida ao se conhecer a outra. Esta medida é
dada pela \emph{informação mútua}\index{informação mútua}, definida como
\begin{definition}[Informação Mútua\index{informação mútua}]\label{def:infmut}
...
a \emph{divergência de Kullback-Leibler}\index{divergência de Kullback-Leibler!relação com informação mútua} entre a distribuição conjunta e o produto
das marginais, ou seja,
\begin{equation}
I(X;Y) = D(p(x,y) \mid\mid p(x)p(y)) .
\end{equation}
Vejamos então a definição de divergência de Kullback-Leibler.
\begin{definition}[Divergência de Kullback-Leibler\index{divergência de Kullback-Leibler}]\label{def:divKL}
...
\begin{align}
D(p \mid\mid q) \: &\triangleq \sum_x p(x) \log \frac{p(x)}{q(x)} \\
        &= \E_p \log \frac{p(X)}{q(X)} .
\end{align}
\end{subequations}
Note que, em geral, a divergência de KL não é simétrica, ou seja, $D(p \mid\mid q) \neq D(q \mid\mid p)$.
\end{definition}
Na definição de acima, adotamos a convenção de que o somatório se dá
sobre o suporte\footnote{O suporte de uma distribuição de probabilidade ou de uma função é
  o conjunto de valores para os quais a função (ou distribuição) assume valores não nulos.
  Formalmente, para uma função $f$, o suporte é definido como $S_f = \{ x \in \mathbb{R} \mid f(x) \neq 0 \}$.
  No contexto de distribuições, é o menor conjunto fechado onde a medida de probabilidade é positiva.}
  da distribuição $p$, ou seja, $S_p$. %Adotamos ainda a convenção
...
\begin{align}
 D(\mu_1 \mid\mid \mu_2) \: &= \sum_{x,y} \mu_1(x,y) \log \frac{\mu_1(x,y)}{\mu_2(x,y)} \\
              &= \sum_{x,y} p(x,y) \log \frac{p(x,y)}{p(x)p(y)} = I(X;Y) .
\end{align}
\end{subequations}
A informação mútua\index{divergência de Kullback-Leibler!relação com informação mútua}
é a ``distância'' entre a distribuição conjunta em $X$ e $Y$ e o
...
Ou seja,
\begin{equation}
p(x)p(y) \: = \argmin_{p'(x,y) \backslash p'(x,y)=p'(x)p'(y)} D(p(x,y) \mid\mid p'(x,y)) .
\end{equation}
...
Seja $x_{1:N} = x_1, \ldots, x_N \in \mathcal{X}$, $N$ observações i.i.d.\footnote{Uma coleção de variáveis aleatórias é
...
\end{equation}
onde $\delta$ é a função de Dirac. De forma equivalente, podemos expressar a distribuição empírica por
\begin{equation}
  \hat{p}(x) = \frac{n(x|x_{1:N})}{N} ,
\end{equation}
...
Maximizar a verossimilhança de $p_\theta(x)$ é equivalente a minimizar a divergência
...
\begin{theorem}[Não negatividade da Divergência\index{divergência de Kullback-Leibler!não negatividade}]\label{thm-divnn}
...
\end{theorem}
\begin{proof}
Se $q(x) = 0$ para algum $x \in S_p$, então $D(p \mid\mid q) = \infty$ e o teorema é verdadeiro
(caso trivial).
Vamos assumir então que $q(x) > 0, \forall x \in S_p$. Teremos então
\begin{subequations}\label{eq:thmDge0}
\begin{align}
 D(p \mid\mid q) \: &= \log e \sum_{x \in S_p} p(x) \ln \frac{p(x)}{q(x)} \\
...
O fato da divergência ser não negativa, nos leva como consequência
a não negatividade da informação mútua, uma vez que esta é um caso específico
de divergência.
\begin{lemma}[Não negatividade da Informação Mútua\index{informação mútua!não negatividade}]
A informação mútua entre duas variáveis aleatórias $X$ e $Y$ é tal que
\begin{equation}
 I(X;Y) \: \geq 0 ,
\end{equation}
com igualdade se e somente se $X \independent Y$, ou seja, $X$ e $Y$ são independentes.
\end{lemma}
\begin{proof}
...
%% scripts to create example
% python3 ulysses_vs_urandom.py 
% 
% R
% source('ulysses_vs_urandom.R')
%
% tail -n +2 codes/ulysses_processed.csv | sort | uniq -c | awk -v N="1000000" '{count = $1; probability = count / N; entropy_term = probability * (log(1/probability) / log(2)); sum_entropy += entropy_term; } END { printf "%.6f\n", sum_entropy; }'
% entropy = 4.207941
%
\begin{example}[Divergência de Kullback-Leibler em Textos e Dados Aleatórios]
  Neste exemplo, usamos a divergência de KL para comparar a
  distribuição de caracteres em um texto natural com a de dados aleatórios,
  usando a distribuição uniforme como referência.

  Consideramos um alfabeto reduzido de 26 letras minúsculas $\mathcal{A} = \{a, b, \ldots, z\}$).
  Para o texto natural, usamos um trecho de \emph{Ulysses}, de James Joyce, filtrando para manter apenas letras
  minúsculas (convertendo maiúsculas em minúsculas e removendo pontuação, espaços, etc.).
  Para os dados aleatórios, usamos bytes gerados por \texttt{/dev/urandom}, mapeados para o
  alfabeto [a-z] via $\text{byte} \mod 26$, onde cada valor é associado a
  uma letra (e.g., $0 \to a$, $1 \to b$, ..., $25 \to z$).

  Para cada fonte de dados (texto e \texttt{/dev/urandom}), extraímos amostras
  de tamanhos $n = 100, 1000, 10000, 1000000$. Estimamos a distribuição empírica 
  $p(x)$ usando a máxima verossimilhança com suavização de Laplace 
  A distribuição uniforme é $q(x) = \frac{1}{26}$ para todo $x \in \mathcal{A}$.

  Calculamos $D(p \mid\mid q)$ para cada amostra e plotamos histogramas das distribuições
  empíricas para visualizar como elas diferem da uniforme.
  Esperamos que, para \texttt{/dev/urandom}, $D(p \mid\mid q)$ seja próximo de zero,
  refletindo a aleatoriedade dos dados, enquanto para o texto, $D(p \mid\mid q)$ 
  permaneça maior que zero devido à estrutura linguística.

  Os histogramas das distribuições são mostrados na \Cref{fig:hist_ulysses_urandom}.
  Para Ulysses, observamos que certas letras (e.g., `e', `t') são mais frequentes,
  refletindo as propriedades do idioma inglês. 
  Para \texttt{/dev/urandom}, as frequências são aproximadamente uniformes.

  \begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/hist_ulysses_urandom.pdf}
    \caption{Histogramas da distribuição empírica de letras em Ulysses e em \texttt{/dev/urandom} para $n = 100, 1000, 10000, 1000000$.}
    \label{fig:hist_ulysses_urandom}
  \end{figure}

  A \Cref{fig:kld} mostra a divergência de KL em função de $n$.
  Para \texttt{/dev/urandom}, $D(p \mid\mid q)$ é pequeno e diminui com $n$,
  indicando que a distribuição empírica se aproxima da uniforme.
  Para Ulysses, $D(p \mid\mid q)$ estabiliza em um valor positivo (aproximadamente 0.5),
  refletindo a estrutura não uniforme do texto. Observe que, para um alfabeto
  de 26 caracteres, a entropia máxima é $\log(26) = 4.7$ bits e, calculando
  o valor da entropia para Ulysses obtemos aproximadamente 4.2.
  A diferença é justamente o valor para o qual a divergência se estabiliza na \Cref{fig:kld}.

  \begin{marginfigure}
    \centering
    \includegraphics[width=\textwidth]{figures/kld_plot.pdf}
    \caption{Divergência de KL entre a distribuição empírica e a uniforme para Ulysses e \texttt{/dev/urandom}.}
    \label{fig:kld}
  \end{marginfigure}
...
\begin{theorem}[Limite Máximo da Entropia -- revisitado\index{entropia!máximo}]\label{thm-limmaxentro}
...
\begin{align}
D(p \mid\mid u) &= \sum_{x \in \mathcal{X}} p(x) \log \frac{p(x)}{u(x)} \nonumber \\
...
\end{align}
Como a entropia relativa é não negativa, $D(p \mid\mid u) \geq 0$, teremos
\begin{equation}
D(p \mid\mid u) = \log \vert \mathcal{X} \vert - H(X) \geq 0 ,
\end{equation}
e assim
\begin{equation}
...
\begin{example}
  ...
  \end{equation}
  Neste caso, teremos\footnote{A série $\sum_{i=1}^{\infty} i 2^{-i}$,
     conhecida como ``Gabriel's Staircase'', é um caso especial da série aritmético-geométrica
     $\sum_{i=1}^{\infty} i x^i$, cuja soma, para $|x| < 1$, é dada por $\frac{x}{(1-x)^2}$.
     Essa fórmula é derivada a partir da série geométrica $\sum_{i=0}^{\infty} x^i = \frac{1}{1-x}$ 
     por diferenciação e multiplicação por $x$. Para $x = \frac{1}{2}$, obtém-se $\sum_{i=1}^{\infty} i 2^{-i} = 2$.
     Nicole Oresme (c. 1323--1382) forneceu uma prova geométrica para essa convergência no século XIV,
     um feito notável para a época~\parencite{oresme2003, wiki:geometric_series, edgar2018}. 
     Para mais detalhes, consulte a entrada sobre séries geométricas na \href{https://en.wikipedia.org/wiki/Geometric_series}{Wikipédia} ou o trabalho de
     Oresme em suas \emph{Questiones super Geometriam Euclidis}.}
  \begin{equation}
...
\begin{example}[\cite{yeung2002}]
  ...
  \begin{equation}
    H(X) = - \sum_{i=1}^{\infty} \sum_{j=1}^{\sfrac{2^{2^i}}{2^i}} 2^{-2^i} \log 2^{-2^i} = \sum_{i=1}^{\infty} 1 \rightarrow \infty ,
  \end{equation}
...
Em seguida, vamos analisar o que ocorre quando tomamos a informação mútua
...
A informação mútua de uma variável aleatória $X$ com ela mesma, a \emph{auto-informação} de $X$\index{informação!auto-informação},
...
Todas essas relações observadas entre entropia, entropia conjunta, entropia condicional e informação mútua
podem ser facilmente representadas através do diagrama de
informação\index{informação!diagrama}, um tipo de digrama de Venn utilizado em
teoria da informação para ilustrar tais relações (veja a \Cref{fig:vennshannon}).
Esta correspondência entre o diagrama de Venn e as medidas de informação
de Shannon não são mera coincidência (veja mais detalhes na seção \nameref{sec:medidasvariacoes}, página \pageref{sec:medidasvariacoes}).
\begin{marginfigure}%
  \includegraphics[width=\linewidth]{figures/info-set.pdf}
  \caption{Diagrama de informação ilustrando as relações entre entropias, entropias condicionais e informação mútua, facilitando a visualização das grandezas informacionais.}
  \label{fig:vennshannon}
\end{marginfigure}

De forma semelhante ao que vimos anteriormente, quando foi introduzido
...
mútua condicional.
\begin{definition}[Informação Mútua Condicional\index{informação mútua!condicional}]\label{def:infmutcond}
...
\begin{proposition}[Regra da Cadeia Generalizada da Entropia\index{entropia!regra da cadeia generalizada}]\label{prop:regracadeiagen}
...
\begin{align}
  \begin{split}
  H(X_1, X_2, \ldots, X_M, X_{M+1}) ={}& H(X_1, X_2, \ldots, X_M) + \\
                                       & H(X_{M+1} | X_1, X_2, \ldots, X_M) \label{eq:dmgenregracadeia1}
  \end{split}\\
  \begin{split}
                                    ={}& \sum_{i=1}^{M} H(X_i|X_1, \ldots, X_{i-1}) + \\
                                       & H(X_{M+1} | X_1, X_2, \ldots, X_M) \label{eq:dmgenregracadeia2}
  \end{split}\\
...
\begin{proposition}[Regra da Cadeia Condicional\index{entropia!regra da cadeia condicional}]\label{pro:regracadeiacond}
...
\begin{proposition}[Regra da Cadeia Condicional Generalizada\index{entropia!regra da cadeia condicional generalizada}]\label{pro:regracadeiacondgen}
...
condicionais. 
\begin{proposition}[Regra da Cadeia da Informação Mútua\index{informação mútua!regra da cadeia}]\label{pro:regracadeiainfmut}
...
\begin{align}
  \begin{split}
  I(X_1, X_2, \ldots, X_N ; Y) ={}& H(X_1, X_2, \ldots, X_N) - \\
                                  & H(X_1, X_2, \ldots, X_N | Y) 
  \end{split}\\
  \begin{split}
         ={}& \sum_{i=1}^{N} \left[ H(X_i | X_1, \ldots, X_{i-1}) - \right.\\
            & \left. H(X_i | X_1, \ldots, X_{i-1}, Y) \right] 
  \end{split}\\
...
Nesta seção, apresentamos a \emph{desigualdade de Jensen}\index{desigualdade de Jensen}, uma relação observada para
...
\begin{definition}[Função Convexa\index{convexidade}]
...
\begin{theorem}[Desigualdade de Jensen\index{desigualdade de Jensen}]\label{thm-desjensen}
Seja $f$ uma função convexa e $X$ uma variável aleatória, então
\begin{equation}\label{eq:desjensen}
\E f(X) \: = \sum_x p(x) f(x) \geq f(\E X) \: = f \left( \sum_x (p(x) \: x) \right)
\end{equation}
\end{theorem}
\begin{proof}
...
Para uma distribuição com $k$ pontos de massa podemos escrever
...
\begin{lemma}[Não negatividade da Divergência de Kullback–Leibler\index{desigualdade de Jensen!não negatividade}]
\begin{equation}
D(p \mid\mid q) \geq 0 \text{ com igualdade se e somente se } p(x) = q(x), \forall x.
\end{equation}
\end{lemma}
\begin{proof}
De forma equivalente, iremos mostrar que $-D(p \mid\mid q) \leq 0$.

Seja $S_p=\{x: p(x) > 0\}$, o suporte de $p$, então
\begin{subequations}\label{dem:divnonneg}
\begin{align}
-D(p \mid\mid q) &= -\sum_x p(x) \log \frac{p(x)}{q(x)} = -\sum_{x \in S_p} p(x) \log \frac{p(x)}{q(x)} \label{dem:divnonneg1}\\
...
\end{align}
\end{subequations}
onde em \ref{dem:divnonneg3} utilizamos a desigualdade de Jensen, \Cref{eq:desjensen}.
\end{proof}



A \emph{desigualdade da soma dos logaritmos}\index{desigualdade da soma dos logaritmos} é derivada da desigualdade de Jensen e da convexidade da função logarítmica.
Esta desigualdade também aparece em algumas demonstrações importantes.

\begin{proposition}[Desigualdade da Soma de Logaritmos]\label{prop:ineq-sum-log}
...
A desigualdade da soma de logaritmos pode ser utilizada para mostrar que $D(p \mid\mid q) \geq 0$.
\begin{proposition}[Não negatividade da Divergência\index{divergência de Kullback-Leibler!não negatividade} -- revisitado]
$D(p \mid\mid q) \geq 0$
\end{proposition}
\begin{proof}
  \begin{subequations}
...
\begin{theorem}[A Entropia Relativa é Convexa no Par\index{divergência de Kullback-Leibler!convexidade}]
...
\begin{align}
    D(\lambda p_1 + (1 - \lambda) p_2 \mid\mid \lambda q_1 + (1- \lambda)q_2) &= \sum_x (\lambda p_1(x) + (1 - \lambda) p_2(x) ) \log \frac{\lambda p_1(x) + (1 - \lambda) p_2(x)}{\lambda q_1(x) + (1 - \lambda) q_2(x)} \label{dem:entrelconvpar1} \\
									...
									&= \lambda D(p_1 \mid\mid q_1) + (1 - \lambda) D(p_2 \mid\mid q_2) \label{dem:entrelconvpar3}
\end{align}
\end{subequations}
...
Utilizamos então a desigualdade da soma de logaritmos (veja a \Cref{eq:dessomalog} da \Cref{prop:ineq-sum-log}) para o caso com apenas dois termos:
\begin{equation}
...
\end{equation}
  o que nos leva ao resultado em \ref{dem:entrelconvpar2}. E usamos novamente a definição de divergência (\Cref{def:divKL}) para obter \ref{dem:entrelconvpar3}.
\end{proof}




\begin{theorem}[A Entropia Relativa é Convexa no Primeiro Argumento\index{divergência de Kullback-Leibler!convexidade}]\label{thm:divklconvp}
A divergência de Kullback-Leibler, $D(p \mid\mid q)$, é convexa em relação a $p$, fixando-se $q$.
...
D(\lambda p_1+(1-\lambda)p_2|q) &= \sum_x \left(\lambda p_1+(1-\lambda)p_2\right)\log\frac{\lambda p_1+(1-\lambda)p_2}{q} \label{dem:divklconv1}\\
				...
				&= \lambda D(p_1 \mid\mid q) + (1-\lambda) D(p_2 \mid\mid q) ,\label{dem:divklconv3}
\end{align}
\end{subequations}
...
\begin{theorem}[A Entropia é Côncava\index{entropia!concavidade}]\label{thm:entropiaconcava}
...
\begin{align}
    H(p) &= - \sum_i p_i \log p_i = - \sum_i p_i \log p_i  + \log \vert \mathcal{X} \vert - \log \vert \mathcal{X} \vert \label{dem:entropiaconcava1}\\
        ...
        &= \underbrace{\log \vert \mathcal{X} \vert}_{\text{constante}} - \underbrace{D(p \mid\mid u)}_{\text{convexo}} \label{dem:entropiaconcava5}
\end{align}
\end{subequations}
\end{proof}
A partir da relação em \ref{dem:entropiaconcava5}, podemos ver a entropia como a similaridade com a distribuição uniforme.
Quanto maior a entropia (menor a divergência $D(p \mid\mid u)$), mais próximo estaremos da distribuição uniforme.
...
\begin{theorem}[Concavidade/Convexidade da Informação Mútua\index{informação mútua!convexidade}\index{informação mútua!concavidade}]\label{thm:concconvinfmut}
...
\begin{align}
    I(X;Y) &= D(p(x,y) \mid\mid p(x)p(y)) \label{dem:concconvinfmutA1} \\
...
onde em \ref{dem:concconvinfmutA1} utilizamos a definição de informação mútua, 
  em \ref{dem:concconvinfmutA2} utilizamos a definição de divergência e aplicamos o Teorema de Bayes\index{teorema!de Bayes} em \ref{dem:concconvinfmutA3}.
...
\section{Desigualdade de Processamento de Dados\index{desigualdade de processamento de dados}}
...
As variáveis aleatórias $X$, $Y$ e $Z$ formam uma \emph{cadeia de Markov}\index{cadeia de Markov} nesta ordem
...
A \emph{Desigualdade do Processamento de Dados}\index{desigualdade de processamento de dados} estabelece que a informação mútua
...
\begin{proposition}[Determinismo e a incerteza nula\index{incerteza!nula}]\label{prop:detincertnull}
$H(X)=0$ se, e somente se, $X$ for determinístico.
\end{proposition}
\begin{proof}
Se $X$ é determinístico, então existe $x^\ast \in \mathcal{X}$ tal que $p(x^\ast) = 1$ e
$p(x) = 0, \forall x \neq x^\ast$. Neste caso, teremos $H(X) = 0$.
Por outro lado, se $X$ não for determinístico, então existe $x^\ast \in \mathcal{X}$ tal que $0 < p(x^\ast) < 1$.
Desta forma, $H(X) > -p(x^\ast) \log p(x^\ast) > 0$. Podemos concluir assim que
$H(X) = 0$ se e somente se $X$ for determinístico.
\end{proof}


\begin{proposition}[Quando $Y$ é uma função de $X$]
$H(Y|X) = 0$\index{entropia condicional!nula} se, e somente se, $Y$ for um função de $X$, ou seja, $Y = g(X)$.
\end{proposition}
\begin{proof}
...
\begin{theorem}[Informação mútua condicional é não nula\index{informação mútua condicional!não nula}]\label{thm:infmutcondnnull}
...
\begin{proof}
  ...
  \begin{subequations}
    \begin{align}
      I(X;Y|Z) &= \sum_{x,y,x} p(x,y,z) \log \frac{p(x,y|z)}{p(x|z) p(y|z)} \\
               ...
               &= \sum_z p(z) D(p_{XY|z} \mid\mid p_{X|z} p(Y|z)) ,
    \end{align}
  \end{subequations}
  ...
  Teremos então
  \begin{equation}
    D(p_{XY|z} \mid\mid p_{X|z} p(Y|z)) \geq 0 .
  \end{equation}
...
\begin{proposition}[Independência e informação mútua nula\index{independência}\index{informação mútua!núla}]\label{prop:indinfmutuanull}
...
\begin{theorem}[Condicionar não aumenta a entropia\index{entropia!condicionar não aumenta}]\label{thm:condnaoaumentaentropia}
\begin{equation}
  H(Y|X) \leq H(Y)
\end{equation}
...
\begin{theorem}[Limite de independência para entropia\index{entropia!limite de independência}]\label{thm:limindent}
...
\begin{theorem}[Monotonicidade da informação mútua\index{informação mútua!monotonicidade}]\label{thm:infmutxyzgeq}
  \begin{equation}
    I(X;Y,Z) \geq I(X;Y) ,
  \end{equation}
...
\begin{definition}[Entropia de Colisão]
  A chamada \emph{entropia de colisão}\index{entropia!de colisão}, ou \emph{entropia
  de Rényi}\index{entropia!de Rényi} de segunda ordem ($n=2$) é definida com
\begin{equation}
  H_2(X) = -\log (Pr(X = X')) .
\end{equation}
\end{definition}
...
\begin{corollary}[Limite inferior da probabilidade de colisão com distribuições distintas]
  ...
  \begin{subequations}
    \begin{align}
      Pr(X=X') &\geq 2^{-H(p) - D(p \mid\mid q)} \\
      Pr(X=X') &\geq 2^{-H(q) - D(q \mid\mid p)}
    \end{align}
  \end{subequations}
  ou seja,
  \begin{equation}
  Pr(X=X') \geq \max \left( 2^{-H(p) - D(p \mid\mid q)} , 2^{-H(q)-D(q \mid\mid p)} \right)
  \end{equation}
\end{corollary}
\begin{proof}
  \begin{subequations}\label{dem:liminfprobcoldisdis}
    \begin{align}
      2^{-H(p) - D(p \mid\mid q)} &= 2^{\sum_x p(x) \log p(x) + \sum_x p(x) \log \frac{q(x)}{p(x)}} \label{dem:liminfprobcoldisdis1}\\
...
A comunicação em um \emph{canal ruidoso}\index{canal!ruidoso} é um cenário clássico em teoria da informação, onde uma mensagem
...
Esse processo forma uma cadeia de Markov\index{cadeia de Markov}: $X \rightarrow Y \rightarrow \hat{X}$.
O processo de estimação está inevitavelmente sujeito a erros Para quantificar esse erro e estabelecer
limites sobre a probabilidade de erro na estimação de $X$, introduziremos a desigualdade de Fano \parencite{fano1961}.

Primeiramente, definiremos o erro na estimação como o evento em que $X \neq \hat{X}$, e a
\emph{probabilidade de erro}\index{probabilidade de erro} será denotada por $P_e = p(X \neq \hat{X})$. Em muitas situações, não é
...
\begin{theorem}[Desigualdade de Fano\index{desigualdade de Fano}]\label{thm:desfano}
...
\end{theorem}
\begin{proof}
  Vamos primeiramente definir uma variável aleatórias associada ao erro na comunicação:
  \begin{equation}
  ...
  \begin{subequations}
    \begin{align}
      \begin{split}
        H(X|\hat{X},E) ={}& p(E=0) \underbrace{H(X|\hat{X},E=0)}_{=0} + \\
                          & p(E=1)H(X|\hat{X},E=1) 
      \end{split}\\
                     ={}& (1-P_e) 0 + P_e H(X|\hat{X},E=1) \\
                     \leq{}& P_e \log (\vert \mathcal{X} \vert - 1) , \label{eq:HXXhEc}
    \end{align}
  \end{subequations}
  ...
  Logo $H(X|\hat{X},E=0)=0$. Usamos ainda que, se conhecemos $\hat{X}$ e existe um erro ($E=1$), então sabemos que $X \neq \hat{X}$,
  logo isto nos deixa com $(\vert \mathcal{X} \vert - 1)$ alternativas.
  Assim, teremos $H(X|\hat{X},E=1) \leq \log (\vert \mathcal{X} \vert - 1)$.
  Aplicando \cref{eq:HXXhEc} %$H(X|\hat{X},E) \leq P_e \log (\vert \mathcal{X} \vert - 1)$ 
  na \Cref{eq:demfanoA}, obtemos agora
...
\begin{example}
  ...
  \begin{subequations}
    \begin{align}
      ...
      &= 1 - \left(  p(y = 1 | x = 1) p(x = 1) + p(y = 2 | x = 2) p(x = 2) \right) \\
  ...
  Pelo Teorema da Desigualdade de Fano\index{teorema!da desigualdade de Fano} temos que
  \begin{equation}
  P_e \geq \frac{H(X|Y) - 1}{\log \left( \vert \mathcal{X} \vert - 1 \right)}
  \end{equation}
  ...
  \begin{subequations}
    \begin{align}
      ...
      \begin{split}
        ={}& - \frac{1}{2} \left( 0.6 \log 0.6 + 4 \times 0.1 \log 0.1 \right) - \\
           & \frac{1}{2} \left( 4 \times 0.1 \log 0.1 + 0.6 \log 0.6 \right) 
      \end{split}\\
        ={}& 1.771 \text{ bits. }
    \end{align}
  \end{subequations}
...
\section{Medida de Informação\index{medida!de informação}}\label{sec:medidasvariacoes}
...
\begin{definition}[Campo\index{medida!campo}]
...
\begin{definition}[Átomo\index{medida!átomo}]
...
\begin{example}
  ...
  \begin{marginfigure}[-5cm]%
    \includegraphics[width=0.6\linewidth]{figures/atoms-n2.pdf}
...
\begin{definition}[Medida com sinal\index{medida!com sinal}]
...
O diagrama de informação é apresentado na Figura \ref{fig:setX1X2}.
  \begin{marginfigure}%
    \includegraphics[width=0.6\linewidth]{figures/setX1X2.pdf}
...
O conjunto universo será considerada como sendo $\Omega = \tilde{X}_1 \cup \tilde{X}_2$.
  ...
  Para as v.a.s $X_1$ e $X_2$, as medidas de informação de Shannon\index{medida!de informação de Shannon} são
  \begin{equation}
...
\end{subequations}
  ...
  É possível então verificar a seguinte correspondência\index{medida!correspondências com as medidas de Shannon} com as medidas de informação de Shannon
  \begin{subequations}
  \begin{align}
