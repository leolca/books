...
process_text <- function(file_path) {
  ...
  # Plot using ggplot2
  p <- ggplot(pair_df, aes(x = char1, y = char2, fill = joint_entropy)) +
    geom_tile() +
    scale_fill_gradient(low = "white", high = "darkblue",
                       name = "Joint Entropy") +
    theme_minimal() +
    labs(title = "Joint Entropy of Character Pairs",
         x = "First Character",
         y = "Second Character",
    ...
    guides(fill = guide_colorbar(title = "Joint Entropy"))
...
# ggsave("joint_entropy_plot.png", plot, width = 10, height = 8, dpi = 300)
# Save as PDF at 7x7 inches
ggsave("joint_entropy.pdf", plot = plot, width = 7, height = 7, device = "pdf")
