...
Na essência da teoria da informação, um dos problemas centrais é o de
reproduzir, em um ponto, uma mensagem selecionada em outro ponto, seja de
maneira exata ou aproximada. Os canais de comunicação modelam a transmissão de
informação através de um meio físico ou lógico, frequentemente sujeito a
imperfeições. O estudo dos canais busca responder a questões fundamentais: como
garantir a confiabilidade da transmissão? Qual é a taxa máxima de comunicação
...
permitem tratar matemáticas o problema e compreender o problema.
...
A codificação de canal é uma técnica essencial para garantir a confiabilidade
...
\begin{definition}[Canal Discreto]
  Um canal discreto é caracterizado por um conjunto finito ou contável de símbolos de entrada $X$,
...
Um canal é dito sem memória se a saída em um dado instante $t$ depende apenas do
...
A taxa de fluxo de informação através de um canal mede a quantidade de
...
A taxa de um sistema de comunicação é definida como a quantidade média de
...
A capacidade de um canal é o limite superior da taxa na qual a informação pode
...
O canal binário sem ruído é o modelo mais simples de canal discreto, onde o
...
Um canal com ruído e saídas sem sobreposição é caracterizado por um alfabeto de
...
O canal de permutação é um modelo determinístico onde o alfabeto de entrada e
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
sendo assim a distribuição uniforme na entrada aquela que máximiza a informação
mútua e alcança assim a capacidade do canal. Este modelo é relevante em
sistemas onde a ordem dos símbolos é alterada, como em algumas aplicações de
criptografia ou redes.
...
O canal binário simétrico (CBS) é um dos modelos mais estudados, com alfabeto
...
O canal binário com apagamento (CBA) possui um alfabeto de entrada 
...
O canal de confusão ternário opera com alfabeto de entrada 
...
\begin{subequations}
  \begin{align}
    H(Y|X) &= \Pr(X=0) \underbrace{H(Y|X=0)}_{=0} + \Pr(X=?) \underbrace{H(Y|X=?)}_{=1} + \Pr(X=1) \underbrace{H(Y|X=1)}_{=0} \\
           &= \Pr(X=?) ,
  \end{align}
\end{subequations}
...
Um canal simétrico é um canal discreto cuja matriz de transição
...
\begin{theorem}[Capacidade de Canais Fracamente Simétricos]
...
constante para todo $y \in \mathcal{Y}$. Seja $\alpha = \sum_{x \in
\mathcal{X}} p(y|x)$ essa constante. Escolher uma distribuição uniforme para
...
A informação mútua é maximizada quando $H(Y) = \log |\mathcal{Y}|$, o que ocorre com a distribuição uniforme de entrada. Portanto, a capacidade é
\begin{equation}
...
Um código $(M,n)$ para um canal discreto $(\mathcal{X}, p(y|x), \mathcal{Y})$ consiste em:
\begin{enumerate}
...
confiabilidade de um código, definimos a probabilidade de erro associada a cada
...
probabilidade de erro para a mensagem $i \in \{1, 2, \ldots, M\}$, denotada por
$\lambda_i$, é dada por
\begin{equation}
...
Para um código $(M,n)$ com probabilidade de erro $\lambda_i$ para cada mensagem $i \in \{1, \ldots, M\}$, a probabilidade máxima de erro, denotada por $\lambda^{(n)}$, é definida como
\begin{equation}
...
Para um código $(M,n)$ com probabilidade de erro $\lambda_i$ para cada mensagem $i \in \{1, \ldots, M\}$, a probabilidade de erro média, denotada por $P_e^{(n)}$, é definida como
\begin{equation}
...
Uma taxa $R$ é dita alcançável para um canal discreto $(\mathcal{X}, p(y|x), \mathcal{Y})$ 
...
A definição de taxa alcançável estabelece que uma taxa $R$ permite transmissão confiável se a probabilidade máxima de erro $\lambda^{(n)}$ tende a zero à medida que o comprimento do código $n$ aumenta. Um conceito central na teoria da informação é a \emph{capacidade do canal}, que determina a taxa máxima para a qual tal confiabilidade é possível. Para canais discretos sem memória, a capacidade marca a fronteira entre taxas viáveis e não viáveis, conforme estabelecido pelo teorema de codificação de canal de Shannon. A seguir, definimos formalmente esse conceito.



\begin{definition}[Capacidade do canal]
A capacidade de um canal discreto sem memória $(\mathcal{X}, p(y|x), \mathcal{Y})$, 
...
erro tenda a zero para taxas abaixo da capacidade. A seguir, definimos
formalmente esse conceito.

\begin{definition}[Tipicidade Conjunta]
...
$(x^n, y^n) \in \mathcal{X}^n \times \mathcal{Y}^n$ é dito tipicamente conjunto 
...
tipicamente conjunto $A_{\epsilon}^{(n)}$ tem aproximadamente $2^{nH(X,Y)}$
...
canal, onde o número de mensagens confiáveis é limitado a aproximadamente
...
Então, para qualquer $\epsilon > 0$, o conjunto tipicamente conjunto $A_{\epsilon}^{(n)}$ satisfaz as seguintes propriedades:
\begin{enumerate}
  \item A probabilidade de $(X^n, Y^n)$ ser tipicamente conjunto tende a 1:
     \begin{equation}
...
probabilidade de erro de cruzamento $p$, a decodificação correta de uma
...
probabilidade $p$ do canal. Pela Propriedade da Equipartição Assintótica, os
erros típicos $E^n$ têm peso de Hamming próximo de $np$, com probabilidade
...
$p(x)$. Analisando a probabilidade média de erro $P_e^{(n)}$ sobre esses
...
\begin{enumerate}
    \item $(x^n(\hat{w}), y^n) \in A_{\epsilon}^{(n)}$, onde $A_{\epsilon}^{(n)}$ é o conjunto tipicamente conjunto;
...
\end{subequations}
para $n$ suficientemente grande.
Como $M \leq 2^{nR} + 1$, para $R < I(X;Y) - 3\epsilon$, o termo $2^{nR} 2^{-n(I(X;Y) - 3\epsilon)} = 2^{-n(I(X;Y) - 3\epsilon - R)}$ tende a zero. 
...
e pelo menos $\sfrac{M}{2} \geq \lceil 2^{nR} \rceil / 2 \geq 2^{n(R - 1/n)}$ palavras
...
A entropia de $W$ satisfaz $H(W) \leq \log M \leq nR + 1$, com igualdade aproximada para $W$ uniforme. Pela identidade da entropia:
\begin{equation}
H(W) = H(W \mid Y^n) + I(W; Y^n).
\end{equation}
Como $P_e^{(n)} \to 0$, a incerteza condicional $H(W \mid Y^n)$ é limitada. Pelo lema de Fano, temos:
\begin{equation}
H(W \mid Y^n) \leq h(P_e^{(n)}) + P_e^{(n)} \log (M - 1),
\end{equation}
onde $h(p)$ é a entropia binária. 
...
Assim:
\begin{equation}
H(W \mid Y^n) \leq \delta_n = h(P_e^{(n)}) + P_e^{(n)} \log (M - 1),
\end{equation}
com $\delta_n \to 0$ à medida que $n \to \infty$. Portanto:
\begin{equation}
...
\end{equation}
Como $H(W) \leq \log M \leq nR + 1$, segue que:
\begin{equation}
nR + 1 \leq nC + \delta_n.
\end{equation}
Dividindo por $n$:
\begin{equation}
R \leq C + \frac{\delta_n}{n} + \frac{1}{n}.
\end{equation}
À medida que $n \to \infty$, $\delta_n/n \to 0$ e $1/n \to 0$, logo $R \leq C$.
...
Na análise de canais de comunicação, é frequentemente necessário relacionar a
probabilidade de erro na decodificação à incerteza remanescente sobre a
mensagem transmitida. O Lema de Fano, também conhecido como desigualdade
de Fano, fornece um limite superior para a entropia condicional $H(W \mid Y^n)$
da mensagem $W$ dado o canal de saída $Y^n$, em termos da probabilidade média
de erro $P_e^{(n)}$. Este resultado é particularmente útil na demonstração da
parte inversa do Teorema de Codificação de Canal de Shannon, onde mostramos
que, se a probabilidade máxima de erro $\lambda^{(n)}$ tende a zero, a taxa $R$
deve ser no máximo igual à capacidade $C$. A seguir, enunciamos o lema de Fano
de forma precisa, destacando sua aplicação a canais discretos sem memória.



\begin{lemma}[Desigualdade de Fano]
...
\begin{equation}
H(W \mid E, Y^n) = \Pr(E = 0) H(W \mid E = 0, Y^n) + \Pr(E = 1) H(W \mid E = 1, Y^n).
\end{equation}
...
\end{equation}
Igualando as duas expressões para $H(E, W \mid Y^n)$:
\begin{equation}
...
de codificação de fonte de Shannon, uma fonte com entropia $H$ pode ser
comprimida sem perda a uma taxa $R \geq H$ bits por símbolo, enquanto o teorema
de codificação de canal estabelece que a transmissão confiável é possível em um
...
Seja $\{V_i\}$ um processo estocástico com alfabeto finito $\mathcal{V}$, que
satisfaz a Propriedade da Equipartição Assintótica (AEP), com taxa de entropia
$H(\mathcal{V}) = \lim_{n \to \infty} \frac{1}{n} H(V_1, \ldots, V_n)$, e seja
$(\mathcal{X}, p(y|x), \mathcal{Y})$ um canal discreto sem memória com capacidade 
$C = \max_{p(x)} I(X; Y)$. Denote $V^n = (V_1, \ldots, V_n)$ a sequência da fonte, codificada
como $X^n = f(V^n)$, transmitida pelo canal, gerando $Y^n$, com a estimativa
...
com erro pequeno, segundo o teorema de canal.
...
probabilidade, sugerindo que $H(V^n \mid Y^n)$ é pequeno. Pelo Lema de Fano:
