...
A \emph{codificação de fonte} sem perdas é o objetivo central quando
...
\begin{definition}[Codificação de fonte\index{código!codificação de fonte}]
...
\begin{definition}[Comprimento esperado]
  O \emph{comprimento esperado}\index{código!comprimento esperado} $L(C)$ de um código $C$ para uma v.a. $X$ com distribuição $p(x)$ é dado por
\begin{equation}
  L(C) = \sum_x p(x) l(x) .
\end{equation}
\end{definition}
...
\begin{definition}[Eficiência de um código]
  A \emph{eficiência de um código}\index{código!eficiência} é definida da seguinte forma
    \begin{equation}
...
\begin{example}
  ...
  \begin{table}[h]
    ...
    \end{tabular}
  \end{table}
  Neste caso, obtemos valores impróprios para o comprimento esperado dos códigos $C_I$ e $C_{II}$,
  indicando que não podem ser códigos sem perdas, já que violam o teorema de Shannon.
  ...
  Utilizando o código $C_{IV}$ é possível decodificar a sequência $00000000111$
  univocamente, porém os símbolos não são instantaneamente decodificados. É
  necessário analisar os bits à frente para conseguir decodificar.
\end{example}


\begin{definition}[Extensão de um código]
  Uma \emph{extensão}\index{código!extensão} $C^\ast$ de um código $C$ é um mapeamento de uma sequência finita
...
Um código é dito \emph{não-singular}\index{código!não-singular} se todo elemento de $\mathcal{X}$
...
ou seja, não pode haver ambiguidade na decodificação.

\begin{definition}[Código com decodificação unívoca]
Um código $C$ com extensão $C^\ast$ é decodificável \emph{univocamente}\index{código!unívoco} se a sua extensão $C^\ast$ for não-singular.
\end{definition}
...
Um código é chamado \emph{código de prefixo}\index{código!de prefixo} ou \emph{código instantâneo}\index{código!instantâneo|see{de prefixo}} se nenhum palavra
...
A \Cref{fig:tiposcodigos} apresenta a estrutura hierárquica dos códigos
...
\begin{marginfigure}[-4cm]%
  ...
  \resizebox{\textwidth}{!}{
  \begin{tikzpicture}
    % Outermost rectangle: Yellow
    % Coordinates are directly specified for each rectangle
    \filldraw[fill=yellow!80, draw=black, thick] (-5, -3) rectangle (5, 3);
    \node[anchor=north west] at (-4.8, 2.8) {todos os códigos}; % Adjusted node position directly

    % Second rectangle: Orange/Salmon
    \filldraw[fill=orange!80!red!70, draw=black, thick] (-4, -2) rectangle (4, 2);
    \node[anchor=north west] at (-3.8, 1.8) {códigos não-singulares};

    % Third rectangle: Light Blue/Cyan
    \filldraw[fill=cyan!60!blue!30, draw=black, thick] (-3, -1.5) rectangle (3, 1);
    \node[anchor=north west] at (-2.8, 0.85) {códigos unicamente decodificáveis};

    % Innermost rectangle: Green
    % This one is not perfectly centered, so its coordinates are different
    \filldraw[fill=green!70!lime, draw=black, thick] (-2, -0.9) rectangle (2, 0.25);
    \node[anchor=north west] at (-1.8, -0.1) {códigos instantâneos};
  \end{tikzpicture}
  }
...
A \emph{desigualdade de Kraft}\index{desigualdade de Kraft} estabelece uma condição necessária e suficiente para a
existência de códigos de prefixo\index{código!de prefixo!desigualdade de Kraft} com comprimentos de palavras-código
...
Note que, ao dizer que existe um código de prefixo com aqueles comprimentos, não
...
\begin{tikzpicture}[
    % Global style for all 'draw' commands
    % This will apply to lines, edges, and node outlines (if drawn)
    every picture/.style={draw=black, thick},
    dotnode/.style={circle, fill, inner sep=2.5pt}, % Style for the black dots
]
    % --- Level 1: Root Node ---
    \node[dotnode] (root) at (0,4) {}; % Place the root node

    % --- Level 2: Children of the root ---
    % First child
    \node[dotnode] (child1) at (-3, 2) {};
    \draw (root) -- (child1) node[midway, left=2pt] {1}; % Edge with label

    % Second child (just a line and label, no dot for simplicity to allow for '...')
    \path (root) -- (-1.5, 2) node[midway, right=2pt] {2}; % Path, not drawn, but for placing label
    \draw (root) -- (-1.5, 2); % Actual line

    % Ellipses for middle branches
    \node at (0, 2) {$\ldots$}; % Visual ellipsis for middle branches

    % Last child (similarly, just a dot and a line for simplicity as it's the last one)
    \node[dotnode] (childD) at (3, 2) {};
    \draw (root) -- (childD) node[midway, right=2pt] {D}; % Edge with label

    % --- Level 3: Children of child1 ---
    % Branches from child1
    \path (child1) -- node[midway, left=2pt] {1} ($(child1) + (-1.5, -1)$);
    \draw (child1) -- ($(child1) + (-1.5, -1)$); % Branch 1
    \path (child1) -- node[midway, right=2pt] {2} ($(child1) + (-0.5, -1)$);
    \draw (child1) -- ($(child1) + (-0.5, -1)$); % Branch 2
    \node at ($(child1) + (0.5, -1)$) {$\ldots$}; % Ellipsis for middle branches
    \path (child1) -- node[midway, right=2pt] {D} ($(child1) + (1.5, -1)$);
    \draw (child1) -- ($(child1) + (1.5, -1)$); % Branch D

    % Ellipses for middle branches at this level between child1 and childD
    \node at (0, 0) {$\vdots$}; % Vertical ellipsis to show more levels/branches

    % --- Level 3: Children of childD ---
    % Branches from childD
    \path (childD) -- node[midway, left=2pt] {1} ($(childD) + (-1.5, -1)$);
    \draw (childD) -- ($(childD) + (-1.5, -1)$); % Branch 1
    \path (childD) -- node[midway, right=2pt] {2} ($(childD) + (-0.5, -1)$);
    \draw (childD) -- ($(childD) + (-0.5, -1)$); % Branch 2
    \node at ($(childD) + (0.5, -1)$) {$\ldots$}; % Ellipsis for middle branches (THIS WAS THE LINE WITH THE ERROR)
    \path (childD) -- node[midway, right=2pt] {D} ($(childD) + (1.5, -1)$);
    \draw (childD) -- ($(childD) + (1.5, -1)$); % Branch D

    % Vertical ellipses for lower levels
    \node at ($(child1) + (0, -2)$) {$\vdots$};
    \node at ($(childD) + (0, -2)$) {$\vdots$};
\end{tikzpicture}
}
...
Designaremos por $l_{\text{max}} = \max_i (l_i)$ o comprimento da palavra mais longa.
...
Como por suposição temos $\sum_{i=1}^{m} D^{-s_i} \leq 1$, podemos associar palavras de código,
com comprimentos dados, a nós na árvore, no nível correspondente, eliminando todos os descendentes e, nesse
processo, nunca iremos exaurir a árvore.
...
\begin{theorem}[Desigualdade de Kraft para códigos infinitos contáveis\index{desigualdade de Kraft!infinitos contáveis}]\label{thm:kraftinfinito}
Seja um conjunto infinito contável de palavras de código que formam um código
de prefixo sobre um alfabeto $D$-ário. Então, os comprimentos
$\{\ell_i\}_{i=1}^\infty$ das palavras satisfazem a desigualdade de Kraft
estendida:
\begin{equation}
\sum_{i=1}^\infty D^{-\ell_i} \leq 1.
\end{equation}
Reciprocamente, dada uma sequência de comprimentos $\{\ell_i\}_{i=1}^\infty$
satisfazendo a desigualdade acima, existe um código de prefixo com esses
comprimentos.
\end{theorem}

\begin{proof}
Seja um código de prefixo infinito contável sobre um alfabeto $D$-ário
$\mathcal{D} = \{0, 1, \dots, D-1\}$. Denote a $i$-ésima palavra de código
por $\mathbf{y}_i = y_{i1} y_{i2} \dots y_{il_i}$, onde $l_i$ é o
comprimento da palavra. Vamos provar a desigualdade de Kraft e a existência
do código.

\paragraph{Prova da desigualdade de Kraft.}
Associamos cada palavra de código $\mathbf{y}_i = y_{i1} y_{i2} \dots y_{il_i}$
a um número real na forma de uma representação fracionária:
\begin{equation}
0.y_{i1} y_{i2} \dots y_{il_i} = \sum_{j=1}^{l_i} y_{ij} D^{-j}.
\end{equation}
Este número define o ponto inicial de um intervalo semiaberto da forma
$[0.y_{i1} y_{i2} \dots y_{il_i}, 0.y_{i1} y_{i2} \dots y_{il_i} + D^{-l_i})$
no intervalo unitário $[0,1)$. O comprimento desse intervalo é exatamente $D^{-l_i}$.

Como o código é de prefixo, nenhuma palavra $\mathbf{y}_i$ é prefixo de outra
palavra $\mathbf{y}_k$ ($i \neq k$). Isso implica que os intervalos
associados às palavras são disjuntos, pois os números no intervalo de
$\mathbf{y}_i$ começam com a sequência $y_{i1} y_{i2} \dots y_{il_i}$, que
não é prefixo de nenhuma outra palavra. Como todos os intervalos estão
contidos em $[0,1)$, a soma dos seus comprimentos satisfaz:
\begin{equation}
\sum_{i=1}^\infty D^{-l_i} \leq 1.
\end{equation}
A organização do intervalos associados ao código de prefixo é exemplificada
na \Cref{fig:kraftintervals}.

\begin{marginfigure}%
    %\includegraphics[width=0.5\textwidth]{images/kraftintervals2.pdf}
    \resizebox{1.2\textwidth}{!}{
    \begin{tikzpicture}[
    dotnode/.style={circle, fill=black, inner sep=2pt}, % Style for the black dots
    open_dotnode/.style={circle, draw=black, inner sep=2pt, fill=white}, % Style for the open dots
    every path/.style={draw=black, thick},             % Global style for all paths
    ]
    % Define the extent of the number line
    \def\xmin{0}  % Interval starts at 0
    \def\xmax{7}  % Represents 1 (or end of interval)
    \def\ymin{0}  % Y-coordinate of the main line

    % --- Main Horizontal Line representing [0,1) ---
    % Draw the main line
    \draw (\xmin, \ymin) -- (\xmax, \ymin);
    % Left vertical bar
    \draw (\xmin, \ymin - 0.2) -- (\xmin, \ymin + 0.2);
    % Right parenthesis
    \node at (\xmax, \ymin) [right=-5pt] {)};

    % Labels for 0 and 1
    % --- CORRECTED LINES HERE ---
    \node[below=4pt of {(\xmin, \ymin)}, font=\small] {0}; % Reference the coordinate (\xmin, \ymin)
    \node[below=4pt of {(\xmax, \ymin)}, font=\small] {1}; % Reference the coordinate (\xmax, \ymin)
    % --- END OF CORRECTED LINES ---

    % --- Disjoint Sub-intervals below the main line ---
    % Sub-interval 1 (connected dots below main line)
    \node[dotnode] (int1_start) at (1, \ymin - 0.5) {}; % Start of first sub-interval
    \node[open_dotnode] (int1_end) at (2.5, \ymin - 0.5) {};  % End of first sub-interval
    \draw (int1_start) -- (int1_end);

    % Sub-interval 2 (connected dots below main line, disjoint from first)
    \node[dotnode] (int2_start) at (4.5, \ymin - 0.5) {}; % Start of second sub-interval
    \node[open_dotnode] (int2_end) at (5.5, \ymin - 0.5) {};    % End of second sub-interval
    \draw (int2_start) -- (int2_end);

     % Sub-interval 3 (connected dots below main line, disjoint from first)
    \node[dotnode] (int3_start) at (2.5, \ymin - 0.75) {}; % Start of second sub-interval
    \node[open_dotnode] (int3_end) at (4.5, \ymin - 0.75) {};    % End of second sub-interval
    \draw (int3_start) -- (int3_end);

    % --- Ellipsis on the far right below the line ---
    \node at (6.25, \ymin - 0.5) {\dots};

    % --- Label with Arrow pointing to the START of the first sub-interval ---
    \node[above=0.8cm of int1_start, align=left] (label_y) {0.$y_1 y_2 y_3 \dots y_{l_i}$};
    \draw[-{Stealth[length=3mm, width=2mm]}, thick] (label_y.south) -- (int1_start.north);

    \end{tikzpicture}
    }
    \caption{Ilustração dos intervalos disjuntos associados a palavras binárias de um código de prefixo, contidos no intervalo $[0,1)$.}
    \label{fig:kraftintervals}
\end{marginfigure}


\paragraph{Exemplos ilustrativos.}
Considere um alfabeto binário ($D=2$, $\mathcal{D} = \{0,1\}$).
Para a palavra $0.1$, temos $0.1 = 1 \times 2^{-1} = \frac{1}{2}$, associada ao intervalo $\left[\frac{1}{2}, 1\right)$.
Para a palavra $0.01$, temos $0.01 = 0 \times 2^{-1} + 1 \times 2^{-2} = \frac{1}{4}$, associada ao intervalo $\left[\frac{1}{4}, \frac{1}{2}\right)$.
Para a palavra $0.001$, temos $0.001 = 1 \times 2^{-3} = \frac{1}{8}$, associada ao intervalo $\left[\frac{1}{8}, \frac{1}{4}\right)$.

Para um alfabeto decimal ($D=10$, $\mathcal{D} = \{0,1,\dots,9\}$).
A palavra $0.157$ corresponde ao intervalo $[0.157, 0.158)$.
A palavra $0.159$ corresponde ao intervalo $[0.159, 0.160)$.

\paragraph{Construção do código de prefixo.}
Para a recíproca, fazemos a construção do código de forma similar ao caso finito.
Dada uma sequência $\{l_i\}_{i=1}^\infty$ tal que
$\sum_{i=1}^\infty D^{-l_i} \leq 1$, podemos construir um código de
prefixo iterativamente. Para simplificar a construção, podemos ordenamos os comprimentos em ordem não decrescente.
Comece escolhendo uma palavra de comprimento $l_1$
Para cada $i \geq 2$, selecione uma
palavra de comprimento $l_i$ que não seja prefixo de nenhuma palavra já
escolhida, o que é possível devido à restrição da soma dos comprimentos dos
intervalos. Este processo garante a existência de um código de prefixo com os
comprimentos especificados.

Portanto, a desigualdade de Kraft é necessária e suficiente para a existência
de um código de prefixo infinito contável.
\end{proof}
...
\section{Em busca do código ótimo}\index{código!ótimo}

Desejamos um código que forneça decodificação instantânea e livre de
ambiguidade, além disso, queremos um código eficiente, que tenha comprimento
esperado mínimo. Desta forma, estamos em busca de um código de prefixo ótimo.

O \emph{comprimento esperado}\index{código!comprimento esperado} é dado por
\begin{equation}
  L(C) = \sum_{i} p_i l_i ,
\end{equation}
...
as derivadas se igualem a zero, ou seja, $\sfrac{\partial J}{\partial l_i} = 0$,
...
Assumindo que seja possível utilizar comprimentos fracionários, os comprimentos
...
Apesar de os comprimentos ótimos, dados na \Cref{eq:codotli}, não serem, a princípio,
inteiros, podemos utilizar a codificação em blocos e assim aproximar do limite
...
Entropia é o \emph{comprimento esperado mínimo}\index{código!comprimento esperado mínimo}. O comprimento esperado $L$ de qualquer código
...
\begin{proof}
  \begin{subequations}
    ...
    \end{align}
  \end{subequations}
  onde utilizamos em \ref{eq:demthm:comprespminA}, para simplificar, a soma de três das quatro parcelas, como é dada na \Cref{eq:demthm:simplificar}; e
  e em \ref{eq:demthm:comprespminB} utilizamos que $c \leq 1$ pois a desigualdade de Kraft deve ser satisfeita,
  onde $c = \sum_i D^{-l_i}$.
  Vejamos a seguir a soma da primeira, segunda e quarta parcelas da soma em \ref{eq:demthm:comprespminA}:
...
O \emph{código de Shannon}\index{código!de Shannon} é um código subótimo que consiste em considerar
...
\begin{theorem}[Limites para o comprimento esperado do código de Shannon\index{código!de Shannon!limites para o comprimento esperado}]
...
Suponha então que o código de Shannon utilize $l(x) = \lceil \log 1/q(x) \rceil$, mas a real
...
\begin{theorem}[Otimalidade competitiva do código de Shannon\index{código!de Shannon!otimalidade competitiva}]\label{thm:otcompcodshannon}
...
\begin{theorem}[Otimalidade competitiva do código de Shannon II\index{código!de Shannon!otimalidade competitiva}]\label{thm:otcompcodshannon2}
...
\section{Desigualdade de Kraft revisitada\index{desigualdade de Kraft}}
...
\begin{lemma}[Expansão multinomial\index{expansão multinomial}]\label{lm:expmultin}
...
\end{lemma}
\begin{proof}
  A demonstração utiliza o \emph{Teorema Multinomial}\index{teorema!multinomial} para expandir a soma, revelando sua equivalência à soma combinatória das contribuições de cada sequência.
  \begin{subequations}
    \begin{align}
...
\begin{theorem}[Desigualdade de Kraft para códigos unicamente decodificáveis\index{desigualdade de Kraft}]
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
onde em \ref{eq:demkraftrevA} utilizamos o resultado do Lema \ref{lm:expmultin} e
...
\section{Código de Huffman\index{código!de Huffman}}
...
para criar um código. A abordagem consistem em: começar pelo topo e 
realizar divisões com entropia máxima para determinar os símbolos.
Em cada etapa dividimos os símbolos em dois grupos, de forma que cada etapa
tenha entropia máxima no processo de determinação dos símbolos.
...
\begin{example}[Método Guloso \emph{top-down} para codificação \parencite{bilmes2013}]
  ...
  na \Cref{fig:greedytree}. Este código possui comprimento esperado $E l = 2.53$ e 
  ...
  \caption{Árvore obtida através do método guloso seguindo a estratégia \emph{top-down} \parencite{bilmes2013}.}\label{fig:greedytree}
  \end{marginfigure}

  A título de comparação, o algoritmo de Huffman produz o código apresentado na \Cref{fig:huffman}.
  O código de Huffman terá comprimento esperado $E l_{\text{huffman}} = 1.97$ e eficiência $H/E l_{\text{huffman}} = 0.9809$.
  ...
  \caption{Árvore obtida pelo código de Huffman \parencite{bilmes2013}.}\label{fig:huffman}
  \end{marginfigure}
\end{example}
...
O procedimento e o código de Huffman obtido estão dispostos na \Cref{fig:ex-huffman01}.
       ...
       \caption{Código de Huffman exemplo \parencite{bilmes2013}.}\label{fig:ex-huffman01}
       \end{figure}
...
\begin{lemma}[Propriedades de códigos instantâneos ótimos\index{código!instantâneo ótimo!propriedade}]\label{lem:propcodinstot}
...
existe um prefixo dela que seja uma outra palavra (veja a \Cref{fig:lemmahuffman}).
 ...
 \begin{tikzpicture}[
  ...
  reddot/.style={circle, fill=red, inner sep=2pt},
  every edge/.style={draw=black, thick}
  ]
  % Left structure: irmãos após apagar
  \node (top1) [dot] at (-1, 2) {};
  \node (mid1) [dot, below=0.5cm of top1] {};
  \node (bottomleft1) [dot, below left=0.5cm of mid1] {};
  \node (bottomright1) [dot, below right=0.5cm of mid1] {};
  \node (red1) [reddot, below=0.7cm of bottomright1] {};

  \draw (top1) -- (mid1);
  \draw (mid1) -- (bottomleft1);
  \draw (mid1) -- (bottomright1);
  \draw [red, thick] (bottomright1) -- (red1);

  \node at (-1, 2.5) {\vdots};
  \node [above=0.1cm of {$(top1.north)!(0,2.5)!(top1.north)$}] {irmãos após apagar};

  % Right structure: não irmãos após apagar
  \node (top2a) [dot] at (3, 2) {};
  \node (bottom2a) [dot, below=0.5cm of top2a] {};
  \node (top2b) [dot, right=1cm of top2a] {};
  \node (bottom2b) [dot, below=0.5cm of top2b] {};
  \node (red2) [reddot, below=0.7cm of bottom2a] {};

  \draw (top2a) -- (bottom2a);
  \draw (top2b) -- (bottom2b);
  \draw [red, thick] (bottom2a) -- (red2);

  \node at (3, 2.5) {\vdots};
  \node [above=0.1cm of {$(top2a.north)!(3,2.5)!(top2a.north)$}] {não irmãos após apagar};
  \end{tikzpicture}
...
Devido à propriedade \ref{propcodinstot1} ($p_k < p_j \Rightarrow l_k \geq l_j$), se $p_k$ é a menor probabilidade, então ela deve
...
%\includegraphics[width=0.5\textwidth]{figures/lemmahuffman2.pdf}
  \begin{tikzpicture}[
    dot/.style={circle, fill=black, inner sep=2pt}, % Style for the black dots
    every edge/.style={draw=black, thick},        % Global style for all lines
    level distance=0.5cm,                            % Vertical distance between levels
    level 1/.style={sibling distance=1cm},         % Horizontal distance at level 1
    % Specific adjustment for level 2 to prevent overlap
    level 2/.style={sibling distance=0.5cm},        % <--- DECREASED SIBLING DISTANCE HERE
  ]
    % --- Left Tree ---
    % Place the root of the left tree
    \node[dot] (rootL) at (-1.5,0) {}; % <--- SLIGHTLY ADJUSTED STARTING X FOR LEFT TREE

    % Level 1 children
    \node[dot, below left of=rootL] (L1_L) {}; % Left child of rootL
    \node[dot, below right of=rootL] (L1_R) {}; % Right child of rootL

    % Level 2 children
    % These will use the level 2 sibling distance
    %\node[dot, below left of=L1_L] (L2_LL) {}; % Left child of L1_L
    \node[dot] (L2_LL) at ($(L1_L) + (-0.3cm, -0.75cm)$) {};
    %\node[dot, below right of=L1_L] (L2_LR) {}; % Right child of L1_L
    \node[dot] (L2_LR) at ($(L1_L) + (+0.3cm, -0.75cm)$) {};
    %\node[dot, below left of=L1_R] (L2_RL) {}; % Left child of L1_R
    \node[dot] (L2_RL) at ($(L1_R) + (-0.3cm, -0.75cm)$) {};
    %\node[dot, below right of=L1_R] (L2_RR) {}; % Right child of L1_R
    \node[dot] (L2_RR) at ($(L1_R) + (+0.3cm, -0.75cm)$) {};

    % Draw edges for the left tree
    \draw (rootL) -- (L1_L);
    \draw (rootL) -- (L1_R);
    \draw (L1_L) -- (L2_LL);
    \draw (L1_L) -- (L2_LR);
    ...
    % Labels for the left tree (rotated)
    % Use rotate=-90 for top-down text
    \node[below=0.5cm of L2_LR, rotate=-90, yshift=3mm] {$p_{m-1}$}; % <--- ROTATED TO -90 AND USED MATH MODE
    \node[below=0.3cm of L2_RR, rotate=-90, yshift=3mm] {$p_m$};     % <--- ROTATED TO -90 AND USED MATH MODE

    % --- Arrow between trees ---
    \draw[thick, -stealth] (0.5,0) -- (2.5,0); % <--- ADJUSTED ARROW POSITION TO BE BETWEEN TREES

    % --- Right Tree (identical structure, shifted) ---
    % Place the root of the right tree
    \node[dot] (rootR) at (4.5,0) {}; % <--- SHIFTED RIGHT TREE CLOSER (OLD: 5, NEW: 6.5)

    % Level 1 children
    \node[dot, below left of=rootR] (R1_L) {}; % Left child of rootR
    \node[dot, below right of=rootR] (R1_R) {}; % Right child of rootR

    % Level 2 children
    %\node[dot, below left of=R1_L] (R2_LL) {}; % Left child of R1_L
    \node[dot] (R2_LL) at ($(R1_L) + (-0.3cm, -0.75cm)$) {};
    %\node[dot, below right of=R1_L] (R2_LR) {}; % Right child of R1_L
    \node[dot] (R2_LR) at ($(R1_L) + (+0.3cm, -0.75cm)$) {};
    %\node[dot, below left of=R1_R] (R2_RL) {}; % Left child of R1_R
    \node[dot] (R2_RL) at ($(R1_R) + (-0.3cm, -0.75cm)$) {};
    %\node[dot, below right of=R1_R] (R2_RR) {}; % Right child of R1_R
    \node[dot] (R2_RR) at ($(R1_R) + (+0.3cm, -0.75cm)$) {};

    % Draw edges for the right tree
    \draw (rootR) -- (R1_L);
    \draw (rootR) -- (R1_R);
    \draw (R1_L) -- (R2_LL);
    \draw (R1_L) -- (R2_LR);
    ...
    % Labels for the right tree (rotated)
    \node[below=0.5cm of R2_LR, rotate=-90, yshift=3mm] {$p_{m-1}$}; % <--- ROTATED TO -90 AND USED MATH MODE
    \node[below=0.3cm of R2_RR, rotate=-90, yshift=3mm] {$p_m$};     % <--- ROTATED TO -90 AND USED MATH MODE

    % --- Additional Text ---
    \node[right=1cm of R1_R, align=left] {diferem apenas\\ no último bit};
  \end{tikzpicture}
...
\begin{theorem}[Otimidade do código de Huffman\index{código!de Huffman!otimalidade}]\label{thm:optcodhuffman}
...
A \emph{codificação Shannon-Fano-Elias}\index{código!Shannon-Fano-Elias} é uma abordagem prática para representar
...
A função de \emph{distribuição cumulativa}\index{distribuição cumulativa}, denotada por $F(x)$, é definida como:
\begin{equation}
...
\cdots < x_m$. A função $\overline{F}(x)$, chamada de \emph{distribuição cumulativa
modificada}\index{distribuição cumulativa!modificada}, é definida como:
\begin{subequations}
\begin{align}
...
um código não singular para $a$ (a expansão binária após a vírgula pode ser utilizada,
como visto anteriormente na prova de Kraft para um infinito contável
de comprimentos, veja o \Cref{thm:kraftinfinito}).  Teremos um código unicamente decodificável, porém poderemos
...
\begin{marginfigure}%
  \includegraphics[width=1.2\linewidth]{figures/intervalshannonfanoelias.pdf}
...
conhecido como o \emph{Jogo de Shannon}\index{jogo de Shannon}. Neste jogo, participantes tentam adivinhar a
...
dependência entre símbolos em uma sequência. Por exemplo, em português, após ``Q'', a 
próxima letra é quase certamente um ``U'', e após ``QU'', a
...
\section{Codificação Aritméticca}

A \emph{codificação aritmética}\index{código!codificação aritmética} supera as limitações dos códigos baseados em símbolos
...
O princípio da codificação aritmética é a sucessiva divisão do intervalo inicial
de probabilidade, $[0, 1)$, com base nas probabilidades dos
símbolos de uma sequência, representando uma mensagem, ao final do processo, como 
um único número nesse intervalo. Para cada novo símbolo, o intervalo atual é
...
\begin{figure}%
  ...
  \caption{Definição dos intervalos probabilísticos para uma sequência de símbolos produzida pela fonte \parencite{mackay2003}.}\label{fig:acoding_invervals}
\end{figure}
...
\begin{marginfigure}%
  ...
  \caption{Definição dos intervalos probabilísticos para uma sequência binária na saída do codificador \parencite{mackay2003}.}\label{fig:acoding_binaryinvervals}
\end{marginfigure}




\begin{example}[Codificação \parencite{mackay2003}]
Suponha $x \in \{a,b,\square\}$, onde $\square$ é o símbolo de término.
Vamos considerar a codificação da sequência $bbba\square$.
...
\begin{marginfigure}%
  ...
  \caption{Intervalo final para a sequências de símbolos da fonte e para a sequência binária na saída do codificador \parencite{mackay2003}.}\label{fig:bbba}
\end{marginfigure}

A \Cref{fig:bbba} ilustra os intervalos que teremos ao final da codificação da sequência $bbba\square$.
Observe que a sequência binária $100111101$ correspondente a um
...
%\begin{lstlisting}[caption={Algoritmo de Codificação Aritmética},label={lst:arithmetic_coding}]
%Algoritmo: Codificação Aritmética
%Entrada: Sequência x_1, ..., x_N; modelo P
%Saída: Intervalo [u, v)
%
%1. u ← 0.0
%2. v ← 1.0
%3. Para n = 1 até N:
%   a. Obtenha P(x_n = a_i | x_1, ..., x_{n-1})
%   b. Q_n ← Σ_{a' < x_n} P(x_n = a' | x_1, ..., x_{n-1})
%   c. R_n ← Σ_{a' ≤ x_n} P(x_n = a' | x_1, ..., x_{n-1})
%   d. v ← u + (v - u) * R_n
%   e. u ← u + (v - u) * Q_n
%4. Retorne [u, v)
%\end{lstlisting}
...
\section{Codificação Lempel--Ziv}

A \emph{codificação Lempel--Ziv}\index{código!Lempel--Ziv}, desenvolvida por \textcite{ziv1977}, marca
...
O algoritmo \emph{LZ77}\index{código!Lempel--Ziv!LZ77} utiliza uma janela deslizante para identificar e substituir
...
de bytes, que contém os próximos símbolos a serem codificados (veja a \Cref{fig:sliding_window_lz77}). 
O codificador
...
A \Cref{fig:lz77_schema} apresenta uma etapa da codificação da sequência
...
que, ao ser acrescentado na \emph{string} de busca, não geraria um \emph{match}.
  ...
  \begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{figures/lz77_schema.png}
...
Ao longo das décadas de 1980 e 1990, o algoritmo LZ77 foi aprimorado de
...
A codificação \emph{LZ78}\index{código!Lempel--Ziv!LZ77}, também conhecida como LZ2, é uma evolução do método
...
aplicações modernas de compressão. O pseudo código para o LZ78 é apresentado
no \Cref{alg:lz78}.
...
\State \quad \( k \gets 1 \) \Comment{Próxima posição livre no dicionário}
\State \quad \( w \gets \) string vazia \Comment{String atual}
\State \quad \( T \gets \) lista vazia \Comment{Lista de tokens}
\For{cada símbolo \( x \) em \( S \)}
    \If{\( w + x \) está em \( D \)}
        \State \( w \gets w + x \)
    \Else
...
\begin{marginfigure}
\centering
\includegraphics[width=1.2\linewidth]{figures/lz78_example.png}
\caption{Exemplo \parencite{salomon2007}.}\label{fig:lz78_example}
\end{marginfigure}

As \Cref{fig:lz78_example,fig:lz78_tree_example} apresentam a aplicação do LZ78
à string exemplo \texttt{sir sid eastman easily teases sea sick seals}.
Na \Cref{fig:lz78_example} é apresentado o dicionário utilizado pelo
algoritmo em cada passo da execução, evidenciando o símbolo de entrada
em cada etapa, a \textit{string} de busca e a saída do codificador.
A \Cref{fig:lz78_tree_example} apresenta a árvore em que a dicionário
é organizado. Note que as sequências de símbolos são armazenadas como
sequências na árvore. Por exemplo, no passo 13 da \Cref{fig:lz78_example},
observamos que a sequência \texttt{sil} deverá ser adicionada à árvore.
Na \Cref{fig:lz78_tree_example}, podemos observar que 13 (sequência \texttt{sil})
foi adicionado abaixo de 5 (sequência \texttt{si}), e esta, por sua vez, abaixo 
de 1 (sequência \texttt{s}), de forma que o caminho percorrido desde a raiz
até o nó 13 irá então representar a sequência \texttt{sil}.
...
A codificação \emph{LZW}\index{código!Lempel--Ziv!LZW}, desenvolvida por \cite{welch1984}, como uma variação do
...
símbolo), o LZW emite apenas o ponteiro para uma \textit{string} no dicionário,
...
\State \quad \( k \gets |A| + 1 \) \Comment{Próxima posição livre no dicionário}
\State \quad \( I \gets \) string vazia \Comment{String atual}
\State \quad \( P \gets \) lista vazia \Comment{Lista de ponteiros}
\For{cada símbolo \( x \) em \( S \)}
    \If{\( I + x \) está em \( D \)}
        \State \( I \gets I + x \)
    \Else
...
dicionário ou reinicializações periódicas. No \Cref{alg:lzw} apresentamos 
o pseudo-código do LZW.

Para o exemplo da codificação da sequência \texttt{sir sid eastman easily teases sea sick seals},
a \Cref{fig:lzw_example} apresenta a saída do codificador e
a \Cref{fig:lzw_dictionary} apresenta o dicionário final.
