...
\chapter{Processos Estocásticos}\label{cht:processosestocasticos}
...
Um \emph{processo estocástico estacionário em sentido estrito}\index{processo estocástico!estacionário em sentido estrito} é uma sequência de
...
\begin{definition}[Processo estocástico estacionário (sentido-estrito)]
  ...
  \emph{estacionária} em sentido estrito se
  \begin{equation}
...
\begin{definition}[Processo de Markov de primeira ordem]
  Um processo estocástico é um \emph{processo de Markov}\index{cadeia de Markov}\index{processo estocástico!Markov|see{cadeia de Markov}} de primeira ordem se
  \begin{equation}
...
\begin{definition}[Cadeia de Markov homogênea\index{cadeia de Markov!homogênea}]
  Uma cadeia de Markov é invariante no tempo (também chamada de \emph{homogênea}) se $p(x_{n+1} \mid x_n)$ não
  depender do tempo, i.e., se
  \begin{equation}
    p(X_{n+1} = b \mid X_{n} = a) = p(X_2 = b \mid X_1 = a) \quad \forall a,b,n .
  \end{equation}
\end{definition}
Neste caso, a cadeia de Markov pode ser descrita por uma matriz de transição fixa $\mathbf{P} = [p_{ij}]_{ij}$
...
estados no instante $n$ e das probabilidades de transição:
\begin{equation}
  p(X_{n+1} = i) = \sum_{j} p(X_n = j) p(i|j) ,
\end{equation}
onde $p(i|j)$ é o elemento $p_{ij}$ da matriz de transição $\mathbf{P}$. Esta cadeia de
...
Quando isto ocorrer, haverá uma distribuição estacionária $\mu$ tal que $\mu^T \mathbf{P} = \mu^T$, onde $\mathbf{P}$ é a matriz de transição.
...
\begin{definition}[Cadeira de Markov irredutível]
  Uma cadeia de Markov é \emph{irredutível}\index{cadeia de Markov!irredutível} se $p_{ij}(n) > 0$ para todo $i,j$ e para algum $n$ onde
...
\begin{definition}[Período de uma cadeia de Markov]
  Uma cadeia de Markov é \emph{periódica}\index{cadeia de Markov!periódica} se $d(i) > 1$ com
  \begin{equation}
  d(i) = \gcd \{ n : p_{ii}(n) > 0 \}
  \end{equation}
...
\begin{example}[Cadeia de Markov homogênea de primeira ordem com dois estados]\label{ex:markovsimples2}
  ...
  \end{marginfigure}
  Se $\mu = [p_1 \ p_2]^T$ é uma distribuição estacionária, então devemos ter $\mu^T \mathbf{P} = \mu^T$.
  Neste caso, teremos
  \begin{subequations}
  \begin{align}
...
utilizada em análises em processamento de sinais; aplicada a equações diferenciais parciais;
...
\begin{lemma}[Média Cesáro\index{média Cesàro}]
...
\begin{proof}
  ...
  \begin{subequations}
  \begin{align}
    \vert b_n - a \vert ={}& \left\vert \frac{1}{n} \sum_{i=1}^n a_i - a \right\vert = \left\vert \frac{1}{n} \sum_{i=1}^n a_i - \frac{1}{n} \sum_{i=1}^n a \right\vert \\
                        ={}& \left\vert \frac{1}{n} \sum_{i=1}^n (a_i - a) \right\vert  \leq  \frac{1}{n} \sum_{i=1}^n \vert a_i - a \vert \label{eq:demmediacesaro}\\
                        ={}& \frac{1}{n} \left( \sum_{i=1}^{N_{\epsilon}} \vert a_i - a \vert + \sum_{i=N_{\epsilon}+1}^n \vert a_i - a \vert \right) \\
                        <{}& \frac{1}{n} \sum_{i=1}^{N_{\epsilon}} \vert a_i - a \vert + \frac{1}{n}  \sum_{i=N_{\epsilon}+1}^n \epsilon \label{eq:demmediacesaroD}\\
  ...
  \end{align}
  \end{subequations}
  onde em \ref{eq:demmediacesaro} utilizamos a desigualdade triangular,
  em \ref{eq:demmediacesaroD} utilizamos a convergência de $a_n$, e em \ref{eq:demmediacesaro2}
...
\begin{definition}[Taxa de entropia de um processo estocástico\index{processo estocástico!taxa de entropia}]
  A \emph{taxa de entropia} de um processo estocástico $\{ X_i \}_i$ é definida como
...
enquanto a \emph{taxa de inovação da informação} $H'(\mathcal{X})$,
...
refletindo a estabilidade estatística do processo ao longo do tempo.
 ...
 \begin{definition}[Taxa de inovação da informação\index{processo estocástico!taxa de inovação da informação}]
 ...
 \begin{theorem}[Um processo estocástico estacionário possui taxa de inovação de informação\index{processo estocástico!taxa de inovação de informação}]
...
\begin{theorem}[Em um processo estocástico estacionário taxa de inovação é igual a taxa de entropia\index{processo estocástico!teorema taxa de inovação e taxa de entropia}]
...
\begin{example}[Taxa de entropia para cadeia de Markov de primeira ordem\index{cadeia de Markov!taxa de entropia}]
...
\begin{subequations}
\begin{align}
\mathbf{\mu}^T \mathbf{P} &= \mathbf{\mu}^T \\
\mathbf{\mu}^T (\mathbf{P}-\mathbf{I}) &= 0 \\
\mathbf{\mu}^T \mathbf{Q} &= 0
\end{align}
\end{subequations}
...
\mathbf{\mu}^T \tilde{\mathbf{Q}} = (0, 0, 0, 1) .
\end{equation}

Para solucionar a Equação \ref{eq-novo-sistema}, iremos pós multiplicar ambos os lados
pela matriz inversa de $\tilde{\mathbf{Q}}$,
\begin{equation}
\mathbf{\mu}^T = (0, 0, 0, 1) \tilde{\mathbf{Q}}^{-1} ,
\end{equation}
obtendo assim
\begin{equation}
...
e $\mathbf{p_i}$ a $i$-ésima linha da matriz $\mathbf{P}$ (as probabilidades de transição à partir
do estado $i$).
