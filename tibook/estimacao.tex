\section{Estimação da Entropia\index{entropia!estimação}}
...
Ao utilizá-la como uma medida de incerteza\index{incerteza} de uma fonte, devemos ter em mente tais limitações. 
...
muito utilizado é a \emph{estimativa de máxima verossimilhança}\index{estimativa de máxima verossimilhança} (MLE), que se relaciona
diretamente com a minimização da divergência de Kullback-Leibler\index{divergência de Kullback-Leibler!minimização}. 
...
via uma  estatística\index{estatística!estimação de distribuição} $\hat{\theta} = T(x_1, \ldots, x_N)$. 
A \emph{verossimilhança}\index{verossimilhança} $L(\theta)$ é a probabilidade conjunta dos dados sob o modelo:
\begin{equation}
...
$N_x = N(x \vert x_1, \ldots, x_N)$ expressa o número de ocorrências do símbolo $x$ na amostra $x_1, \ldots, x_N$.
...
\begin{subequations}
  \begin{align}
    D_{\text{KL}}(\hat{p} \| p_\theta) &= \sum_{x \in \mathcal{X}} \hat{p}(x) \log \frac{\hat{p}(x)}{p_\theta(x)} \\
                                       ...
                                       &= -H(\hat{p}) - \frac{1}{N} \ell(\theta) .
  \end{align}
\end{subequations}
Como $H(\hat{p})$ é constante para um conjunto fixo de dados, minimizar 
$D_{\text{KL}}(\hat{p} \| p_\theta)$ equivale a maximizar $\ell(\theta)$. 
...
estimativa da distribuição, especialmente em fontes com baixa probabilidade para certos
símbolos. Para abordar essa questão, técnicas de \emph{suavização}\index{suavização}
(em inglês, \textit{smoothing}) ajustam as probabilidades empíricas, atribuindo valores
...
A \emph{Regra de Sucessão de Laplace}\index{suavização!regra de sucessão de Laplace}, proposta no século XVIII por Pierre-Simon
...
Essa regra suaviza as estimativas ao assumir uma distribuição \textit{a priori} uniforme
sobre a probabilidade $p_0$ no intervalo $[0, 1]$.
Isso reflete a suposição de ignorância total sobre a distribuição,
antes de observar os dados.

Considere uma fonte binária com alfabeto $\mathcal{X} = \{0, 1\}$, onde
$X_1, \ldots, X_N$ são variáveis aleatórias i.i.d. com $p(0) = p_0$ e  
...
\end{equation}
Assumimos uma distribuição \textit{a priori} uniforme para $p_0$, ou seja, $\Pr(p_0) = 1$ para $p_0 \in [0, 1]$. 
%refletindo incerteza total sobre a distribuição. Pelo teorema de Bayes, a distribuição \textit{a posteriori} de $p_0$ é:
\begin{subequations}\label{eq:Prp0givenx1n}
  \begin{align}
...
O denominador da \Cref{eq:Prp0givenx1n}, a probabilidade marginal dos dados, é calculado usando a função Beta\footnote[][-30ex]{
A função Beta é definida como:
\begin{equation}
  B(a,b) = \int_0^1 x^{a-1} (1-x)^{b-1} \diff x.
\end{equation}
...
\begin{subequations}
  \begin{align}
    \Pr(x_1, \ldots, x_N) &= \int_0^1 \Pr (x_{1:N}, p_0) \diff p_0 \label{eq:demprobmargbetaA}\\
                          &= \int_0^1 p_0^{N_0} p_1^{N_1} \Pr(p_0) \diff p_0 \label{eq:demprobmargbetaB}\\
                          &= \int_0^1 p_0^{N_0} p_1^{N_1} \diff p_0 \label{eq:demprobmargbetaC} \\
                          &= B(N_0 + 1, N_1 + 1) \\
                          &= \frac{\Gamma(N_0 + 1) \Gamma(N_1 + 1)}{\Gamma(N_0 + N_1 + 2)} \label{eq:demprobmargbetaE} \\
...
Em \ref{eq:demprobmargbetaC} utilizamos que a distribuição \textit{a priori} é uniforme, $\Pr(p_0)=1$.
A integral remanescente em \ref{eq:demprobmargbetaC} é característica da função Beta.
Em \ref{eq:demprobmargbetaE}, avaliando a função gamma nos inteiros positivos $n$, temos $\Gamma(n) = (n-1)!$.
Para prever a probabilidade do próximo símbolo ser 0, calculamos:
\begin{subequations}
  \begin{align}
    \Pr(X_{N+1} = 0 | x_1, \ldots, x_N) &= \int_0^1 p_0 \Pr(p_0 | x_1, \ldots, x_N) \diff p_0 \\
                                        &= \int_0^1 p_0 \frac{p_0^{N_0} (1 - p_0)^{N_1}}{\Pr(x_1, \ldots, x_N)} \diff p_0 \\
                                        &= \frac{\int_0^1 p_0^{N_0 + 1} (1 - p_0)^{N_1} dp_0}{\int_0^1 p_0^{N_0} (1 - p_0)^{N_1} \diff p_0} \\
...
A Regra de Laplace assume uma distribuição \textit{a priori} uniforme, equivalente a uma distribuição
Beta com parâmetros $\alpha_0 = \alpha_1 = 1$. A abordagem de Dirichlet\index{suavização!regra de Dirichlet}
generaliza isso, permitindo distribuições \textit{a priori} mais flexíveis. Para uma fonte binária, a
distribuição \textit{a priori} é uma distribuição Beta com parâmetros $\alpha_0, \alpha_1 > 0$, e a
...
outros valores ajustam o peso da distribuição \textit{a priori}, útil em contextos onde há conhecimento prévio sobre a fonte. 
Para alfabetos maiores, a regra de Dirichlet estende a suavização a múltiplos símbolos.
...
essas probabilidades empíricas. No entanto, há vários fatores que devemos
...
O método mais comum para estimar a entropia é o \emph{estimador plug-in}\index{entropia!estimador \textit{plug-in}}, que
...
\label{eq:mle-prob}
\end{equation}
é o número de ocorrências de $a_k$ na amostra. %, e $\delta(x_n = a_k)$ é a delta de Kronecker ($1$ se $x_n = a_k$, $0$ caso contrário). 
A entropia estimada é então:
\begin{equation}
...
distribuídos (i.i.d.), ou seja, não há dependências entre símbolos
...
Para mitigar o problema de eventos raros, técnicas de \emph{suavização}\index{suavização} atribuem
...
sistematizaram métodos de suavização.\footnote[][-27ex]{As técnicas de suavização foram
...
O estimador \textit{plug-in}\index{entropia!estimador \textit{plug-in}} é não paramétrico, intuitivo e apresenta
...
\label{eq:variance}
\end{equation}
Essas expressões, originalmente em nats \parencite{harris1975}, devem ser convertidas 
...
O estimador de \cite{miller1955}\index{entropia!estimador de Miller} ajusta o viés do estimador
\textit{plug-in} adicionando um termo de correção baseado no número de símbolos
observados. Dada a entropia \textit{plug-in}, $\hat{H}$ (definida na \Cref{eq:estpluginentropia}), 
...
\begin{equation}
\hat{p}_{k,-i} = \frac{n_k - \delta(x_i = a_k)}{N - 1}.
\end{equation}
...
sem abordar o problema da interdependência de símbolos. Aqui, apresentamos apenas alguns
...
\\[-1.8ex]\hline
\hline \\[-1.8ex]
\multicolumn{1}{l}{Métrica} & \multicolumn{1}{r}{Plug-in} & \multicolumn{1}{r}{Laplace} & \multicolumn{1}{r}{Miller} & \multicolumn{1}{r}{Laplace-Miller} \\
\hline \\[-1.8ex]
\multicolumn{1}{l}{Entropia por caractere (bits/char)} & 4.2467 & 4.2467 & 4.2467 & 4.2468 \\
\multicolumn{1}{l}{Entropia por palavra (bits/word)} & 10.6757 & 11.2962 & 10.7332 & 11.3537 \\
\multicolumn{1}{l}{Entropia por caractere por palavra (bits/char)} & 2.3687 & 2.5064 & 2.3815 & 2.5192 \\
\multicolumn{1}{l}{Comprimento esperado de palavra (chars)} & 4.5069 & 4.5069 & 4.5069 & 4.5069 \\
\hline \\[-1.8ex]
\end{tabular}
